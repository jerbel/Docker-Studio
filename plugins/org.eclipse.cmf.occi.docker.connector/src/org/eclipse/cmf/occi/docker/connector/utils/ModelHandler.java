...
// TODO : Refactor with OCCIHelper and synchronize / import model commands..
	...
	public void loadModelWithoutGenerateCode() {
		...
		for (final EClassifier eClassifier : eClassifiers) {
			LOGGER.info("Classifier: " + eClassifier.getName());
			LOGGER.info(" ");
			// Run through classes
			if (eClassifier instanceof EClass) {
				...
				for (EAttribute eAttribute : eAttributes) {
					LOGGER.info(eAttribute.getName() + "(" + eAttribute.getEAttributeType().getName() + ") ");
				}
				...
				for (EReference eReference : eReferences) {
					LOGGER.info(eReference.getName() + "(" + eReference.getEReferenceType().getName() + "["
							+ eReference.getLowerBound() + ".." + eReference.getUpperBound() + "])");
				}
				if (!eClass.getEOperations().isEmpty()) {
					LOGGER.info(" Operations: ");
					for (EOperation eOperation : eClass.getEOperations()) {
						LOGGER.info(eOperation.getEType() + " " + eOperation.getName());
					}
				}
			}
		}
	...
	public void createAndSaveDockerMachineWithMetaModel() {
		// TODO : To refactor using OCCI core factories and facility with OCCIHelper..
		try {
			...
			EClassifier _eClassifier_1 = ePackage.getEClassifier("MachinevirtualBox");
			final EClass evbox = ((EClass) _eClassifier_1);
			EStructuralFeature _eStructuralFeature_2 = evbox.getEStructuralFeature("boot2dockerurl");
			final EAttribute eboot2docker_url = ((EAttribute) _eStructuralFeature_2);
			EStructuralFeature _eStructuralFeature_3 = evbox.getEStructuralFeature("disksize");
	...
	public void saveFromStream(final OutputStream output, final Machine machine) {
	  ...
	  public Machine loadFromStream(final InputStream input) {
	    try {
	      ...
	      return ((Machine) _get);
	    		...
	    		LOGGER.error(" Exception : " + _e.getMessage());
		    _e.printStackTrace();
		    return null; // TODO : Manage Exceptions from emf models manipulation.
	    }
	  }
	      ...
	      try {
	        ...
	        if (_t instanceof IOException) {
	          ...
	          // throw Exceptions.sneakyThrow(_t);
	        	  // throw new DockerModelException(); // TODO: Manage model exception.
	        		throw new NullPointerException(_t.getMessage());
	        }
	      }
	      return result;
	    } catch (Throwable _e) {
	      // throw Exceptions.sneakyThrow(_e);
	    	  throw new NullPointerException(_e.getMessage()); // TODO : Manage Model Exception.
	    }
	  }
	  ...
	  public Machine getModel(final String machine, final String state, final boolean machineExists) {
	    ...
	    String _inspectHostCmd = DockerMachineHelper.inspectHostCmd(_runtime, machine);
	    ...
	    if (_notEquals) {
	      ...
	      if ((vbox instanceof Machinevirtualbox)) {
	        ...
	        if ((vbox instanceof Machineamazonec2)) {
	         	Machineamazonec2 newvbox_1 = ((Machineamazonec2) vbox);
	          this.machineFactory(newvbox_1, node, state);
	          ModelHandler.LOGGER.info(("Model setting: " + newvbox_1));
	        } else {
	          if ((vbox instanceof Machinedigitalocean)) {
	        	  Machinedigitalocean newvbox_2 = ((Machinedigitalocean) vbox);
	            this.machineFactory(newvbox_2, node, state);
	            ModelHandler.LOGGER.info(("Model setting: " + newvbox_2));
	          } else {
	            if ((vbox instanceof Machinevmwarefusion)) {
	            	Machinevmwarefusion newvbox_3 = ((Machinevmwarefusion) vbox);
	              this.machineFactory_Fusion(newvbox_3, node, state);
	              ModelHandler.LOGGER.info(("Model setting: " + newvbox_3));
	            } else {
	              if ((vbox instanceof Machinegooglecomputeengine)) {
	            	  Machinegooglecomputeengine newvbox_4 = ((Machinegooglecomputeengine) vbox);
	                this.machineFactory(newvbox_4, node, state);
	                ModelHandler.LOGGER.info(("Model setting: " + newvbox_4));
	              } else {
	                if ((vbox instanceof Machineibmsoftlayer)) {
	                	Machineibmsoftlayer newvbox_5 = ((Machineibmsoftlayer) vbox);
	                  this.machineFactory(newvbox_5, node, state);
	                  ModelHandler.LOGGER.info(("Model setting: " + newvbox_5));
	                } else {
	                  if ((vbox instanceof Machinemicrosoftazure)) {
	                	  Machinemicrosoftazure newvbox_6 = ((Machinemicrosoftazure) vbox);
	                    this.machineFactory(newvbox_6, node, state);
	                    ModelHandler.LOGGER.info(("Model setting: " + newvbox_6));
	                  } else {
	                    if ((vbox instanceof Machinemicrosofthyperv)) {
	                    	Machinemicrosofthyperv newvbox_7 = ((Machinemicrosofthyperv) vbox);
	                      this.machineFactory(newvbox_7, node, state);
	                      ModelHandler.LOGGER.info(("Model setting: " + newvbox_7));
	                    } else {
	                      if ((vbox instanceof Machineopenstack)) {
	                    	  Machineopenstack newvbox_8 = ((Machineopenstack) vbox);
	                        this.machineFactory_OpenStack(newvbox_8, node, state);
	                        ModelHandler.LOGGER.info(("Model setting: " + newvbox_8));
	                      } else {
	                        if ((vbox instanceof Machinerackspace)) {
	                        	Machinerackspace newvbox_9 = ((Machinerackspace) vbox);
	                          this.machineFactory(newvbox_9, node, state);
	                          ModelHandler.LOGGER.info(("Model setting: " + newvbox_9));
	                        } else {
	                          if ((vbox instanceof Machinevmwarevsphere)) {
	                        	  Machinevmwarevsphere newvbox_10 = ((Machinevmwarevsphere) vbox);
	      ...
	      if ((!machineExists)) {
	        ComputeStatus _state = vbox.getOcciComputeState();
	        ...
	        if (_equalsIgnoreCase) {
	          ...
	          if (_notEquals_1) {
	            List<org.eclipse.cmf.occi.docker.Container> modelContainers = this.buildContainer(vbox, containers);
	            for (final org.eclipse.cmf.occi.docker.Container container : modelContainers) {
	              {
	  ...
	  public void machineFactory(final Machine vbox, final JsonNode node, final String state) {
	    ...
	    vbox.setOcciComputeMemory(_parseFloat);
	    ...
	    vbox.setOcciComputeCores(_parseInt);
	    boolean _equals = Objects.equal(state, "Running");
	    if (_equals) {
	      ComputeStatus _get_6 = ComputeStatus.get(0);
	      vbox.setOcciComputeState(_get_6);
	    }
	    boolean _equals_1 = Objects.equal(state, "Stopped");
	    if (_equals_1) {
	      ComputeStatus _get_7 = ComputeStatus.get(1);
	      vbox.setOcciComputeState(_get_7);
	    }
	  }
	  
	  public void machineFactory_VBOX(final Machinevirtualbox vbox, final JsonNode node, final String state) {
	    JsonNode _get = node.get("Driver");
	    JsonNode _get_1 = _get.get("MachineName");
	    String _string = _get_1.toString();
	    String _replaceAll = _string.replaceAll("\"", "");
	    ...
	    vbox.setOcciComputeCores(_parseInt_1);
	    ...
	    vbox.setBoot2dockerURL(_replaceAll_1);
	    boolean _equals = Objects.equal(state, "Running");
	    if (_equals) {
	      ComputeStatus _get_10 = ComputeStatus.get(0);
	      vbox.setOcciComputeState(_get_10);
	    }
	    boolean _equals_1 = Objects.equal(state, "Stopped");
	    if (_equals_1) {
	      ComputeStatus _get_11 = ComputeStatus.get(1);
	      vbox.setOcciComputeState(_get_11);
	    }
	  }
